...
SELECT 
  (SELECT COUNT(DISTINCT user_id) FROM auth.users) as total_users,
...
-- Insert sample orders for testing (optional - can be removed in production)
INSERT INTO public.orders (user_id, restaurant_name, restaurant_id, total_amount, status, delivery_address, payment_method)
SELECT 
  auth.uid(),
  'Sample Restaurant ' || generate_series,
  'rest_' || generate_series,
  (20 + random() * 80)::decimal(10,2),
  CASE 
    WHEN generate_series % 4 = 0 THEN 'delivered'
    WHEN generate_series % 4 = 1 THEN 'preparing'
    WHEN generate_series % 4 = 2 THEN 'delivering'
    ELSE 'confirmed'
  END,
  '123 Main St, Apt ' || generate_series,
  CASE WHEN generate_series % 2 = 0 THEN 'card' ELSE 'paypal' END
